...
def _deproject(x, y, dx0=0.0, dy0=0.0, inc=0.0, PA=0.0, zr=0.0, z_func=None, 
               frontside_only=False, backside_only=False, min_zr=0.0): # [new line 11/03/2025]
    """
    ...
    Args:
        ...
        zr (optional[float]): Uppermost z/r value to assume for the emission.
        z_func (optional[callable]): A user-defined emission height function
            ...
            in [arcsec].
        frontside_only (optional[bool]): If True, only return deprojected 
            coordinates for the front side of the disk (z ≥ min_z).
        backside_only (optional[bool]): If True, only return deprojected 
            coordinates for the back side of the disk (z ≤ -min_z).
        min_zr (optional[float]): The minimum z/r value to include. Defaults 
            to 0.0 (midplane). For example, setting `min_zr=0.1` with 
            `frontside_only=True` will exclude z/r < 0.1.
    ...
    # Initial logic check 
    if frontside_only and backside_only: # [new line 11/03/2025]
        raise ValueError("Cannot set both `frontside_only` and `backside_only`.") # [new line   11/03/2025]
    if min_zr < 0.0: # [new line 11/03/2025]
        raise ValueError("`min_zr` must be non-negative.") # [new line 11/03/2025]
    ...
    # Apply frontside/backside filtering with min_zr threshold
    min_z = min_zr * r_tmp  # Convert min_zr to physical height  # [new line   11/03/2025]
    if frontside_only: # [new line   11/03/2025]
        z_tmp = np.where(z_tmp >= min_z, z_tmp, np.nan)  # Mask out values < min_z # [new line   11/03/2025]
    elif backside_only: # [new line   11/03/2025]
        z_tmp = np.where(z_tmp <= -min_z, z_tmp, np.nan)  # Mask out values > -min_z # [new line   11/03/2025]
...
def _make_zr_list(zr, max_dzr=0.1, frontside_only=False, backside_only=False, # [new line 11/03/2025]
                  min_zr=0.0): # [new line 11/03/2025]
    """List of equally spaced z/r heights with a minimum spacing max_dzr
       starting at min_zr.""" # [new line 11/03/2025]

    if zr == 0.0:
        return np.zeros(1)

    a = np.arange(min_zr, zr, max_dzr) # [new line 11/03/2025]
    a = np.append(a, zr) if a[-1] != zr else a

    if frontside_only and backside_only:
        raise ValueError("Cannot set both frontside_only and backside_only to True.")

    # a = np.concatenate([-a[1:][::-1], a]) # [previous line]
    if not frontside_only and not backside_only: # [new line 11/03/2025]
        a = np.concatenate([-a[1:][::-1], a])  # Default: Include both sides # [new line 11/03/2025]
    elif backside_only: # [new line 11/03/2025]
        a = -a[::-1]  # Keep only the negative side # [new line 11/03/2025]
    # Else, frontside_only is True, and we do nothing (keep only positive side) # [new line 11/03/2025]
...
def make_mask(inc, PA, dist, mstar, vlsr, dx0=0.0, dy0=0.0, zr=0.0,
    ...
    Args:
        ...
        frontside_only (optional[bool]): If True, only mask the front side of 
            the disk (positive z/r values). Defaults to False. 
        backside_only (optional[bool]): If True, only mask the back side of the 
            disk (negative z/r values). Defaults to False. 
        min_zr (optional[float]): Minimum z/r to include. Must be given positive. 
            Default is 0.0.
    ...
    # Initial logic check
    if frontside_only and backside_only: # [new line 11/03/2025]
        raise ValueError("Cannot set both frontside_only and backside_only to True.") # [new line 11/03/2025]
    if min_zr < 0: # [new line 11/03/2025]
        raise ValueError("min_zr must be a positive value.") # [new line 11/03/2025]

    # Define the image axes.
    if image is not None:
        image = image if image[-1] != '/' else image[:-1]
        img_x_axis, img_y_axis, img_s_axis, img_v_axis = _read_image_axes(image)
    x_axis = x_axis if x_axis is not None else img_x_axis
    y_axis = y_axis if y_axis is not None else img_y_axis
    s_axis = s_axis if s_axis is not None else img_s_axis
    v_axis = v_axis if v_axis is not None else img_v_axis
    ...
    mask = None
    # zr_list = _make_zr_list(zr, max_dzr) if z_func is None else [-1., 0., 1.] # [previous line]
    if z_func is None: #[new line 11/03/2025]
        zr_list = _make_zr_list(zr, max_dzr, frontside_only, backside_only, min_zr) #[new line 11/03/2025]
    else: #[new line 11/03/2025]
        if frontside_only: #[new line 11/03/2025]
            zr_list = [min_zr, 1.0]  # Ensure min_zr is the lowest value #[new line 11/03/2025]
        elif backside_only: #[new line 11/03/2025]
            zr_list = [-1.0, -min_zr]  # Ensure -min_zr is the highest value #[new line 11/03/2025]
        else: #[new line 11/03/2025]
            zr_list = [-1.0, 0.0, 1.0]  # Default behavior #[new line 11/03/2025]
    
    offsets = _get_offsets(image, restfreqs) if image is not None else [0.0]
    print('offsets: ', offsets)
    for offset in offsets:
        print('offset: ', offset)
        # offsets = [offset for offset in offsets if np.min(v_axis) <= offset <= np.max(v_axis)]
        for zr in zr_list:
